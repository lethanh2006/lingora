  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <h1 className="mt-1 text-3xl font-bold tracking-tight">Bạn muốn học chủ đề nào?</h1>
        <p className="mt-2 max-w-2xl text-muted-foreground">Mỗi chủ đề dùng chung một danh sách từ cho lật thẻ, ghép từ và điền từ.</p>
      </header>
      {topics.length === 0 ? <CatalogEmptyState icon={LibraryBig} title="Chưa có chủ đề sẵn sàng" description="Quản trị viên chưa bật hiển thị chủ đề từ vựng nào." /> : <div className="grid gap-5 lg:grid-cols-2">{topics.map((topic) => <TopicCard key={topic.id} topic={topic} progress={progressByTopic.get(topic.id)} />)}</div>}
    </div>
  );
}
